    ...
    if(justify()){
        ...
        if(flag === -1){
            ...
            if(userInfo[flag].password=== password){
                ...
                UserSession.lists = userInfo[flag].lists
...
router.post('/delete_schedule', urlencodedParser, function(req, res) {
    ...
    for(var i=0;i<userInfo.length;i++)
        if(userInfo[i].username === UserSession.username){
            console.log('清单:"'+userInfo[i].lists[kindOfList].listName+'"待办事项："'+userInfo[i].lists[kindOfList].schedule[rightRightKindOfSch].sch+'"删除成功')
            userInfo[i].lists[kindOfList].schedule.splice(rightRightKindOfSch,1)
        }
        localStorage.setItem('users', JSON.stringify(userInfo)); 
})
//修改待办事项
router.post('/reviseSch', urlencodedParser, function(req, res) {
    var rightRightKindOfSch = req.body.rightRightKindOfSch;
    var kindOfList = req.body.kindOfList;
    ...
    for(var i=0;i<userInfo.length;i++)
        if(userInfo[i].username === UserSession.username){
            console.log("清单"+userInfo[i].lists[kindOfList].schedule[rightRightKindOfSch].sch+"修改成功")
            userInfo[i].lists[kindOfList].schedule[rightRightKindOfSch].sch = schName
            console.log(date)
            if(date!==undefined){
                userInfo[i].lists[kindOfList].schedule[rightRightKindOfSch].date = date; 
                console.log("清单"+userInfo[i].lists[kindOfList].schedule[rightRightKindOfSch].sch+"时间修改成功")
            }
    ...
    for(var i=0;i<userInfo.length;i++)
        if(userInfo[i].username === UserSession.username){
            ...
            if(userInfo[i].lists[kindOfList].schedule[rightRightKindOfSch].flag===true){
                var obj = userInfo[i].lists[kindOfList].schedule[rightRightKindOfSch]
                userInfo[i].lists[2].schedule.push(obj)
                console.log('待办事项:"'+userInfo[i].lists[kindOfList].schedule[rightRightKindOfSch].sch+'"完成')
            }
            else{
                for(var j=0;j<userInfo[i].lists[2].schedule.length;j++){
                    if(userInfo[i].lists[2].schedule[j].sch==userInfo[i].lists[kindOfList].schedule[rightRightKindOfSch].sch)
                    userInfo[i].lists[2].schedule.splice(j,1)
                }   
